...
static int probe(struct pci_dev *pdev, const struct pci_device_id *id)	/* New device inserted */
{
    ...
    WriteRegister(deviceNumber, kVRegBaseFirmwareDeviceID, (ULWord)ntv2pp->_DeviceID, NO_MASK, NO_SHIFT);
    ...
    if (getNTV2ModuleParams()->driverMode == eDriverModeAll)
    {
        ...
        if (ntv2pp->_DeviceID == DEVICE_ID_KONAIP_25G || ntv2pp->_DeviceID == DEVICE_ID_KONAIP_25G_8CH)
        {
            ...
            if (ntv2pp->m_pRasterMonitor != NULL)
            {
                status = ntv2_videoraster_configure(ntv2pp->m_pRasterMonitor, 0x3400, 64, 
				(ntv2pp->_DeviceID == DEVICE_ID_KONAIP_25G_8CH) ? 8 : 4);
                if (status != NTV2_STATUS_SUCCESS)
                {
                    ntv2_videoraster_close(ntv2pp->m_pRasterMonitor);
                    ntv2pp->m_pRasterMonitor = NULL;
...
static void remove(struct pci_dev *pdev)
{
    ...
    if (ntv2pp->m_pGenlockMonitor != NULL)
    {
        ntv2_genlock_disable(ntv2pp->m_pGenlockMonitor);
        ntv2_genlock_close(ntv2pp->m_pGenlockMonitor);
        ntv2pp->m_pGenlockMonitor = NULL;
    }
...
//    pci_set_master(pdev);
    ...
    if (getNTV2ModuleParams()->driverMode == eDriverModeAll)
    {
        ...
        if (ntv2pp->m_pGenlockMonitor != NULL)
        {
            ntv2_genlock_enable(ntv2pp->m_pGenlockMonitor);
        }
